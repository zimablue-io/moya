...
export function applyLocalIntent(userText: string, world: World): { spoken: string } {
  ...
  if (remember?.[1]) {
    executeBuiltin(
      "memory_write",
      JSON.stringify({ kind: "preference", text: remember[1].trim() }),
      world,
    );
  ...
  if (/needs? me|loop me in|don't let me forget|remind me/.test(lower)) {
    const title = q
      .replace(/^.*?(?:remind me to|loop me in(?: on)?|don't let me forget)\s+/i, "")
      .slice(0, 80);
    executeBuiltin(
      "inbox_add",
      JSON.stringify({ title: title || "Needs you", body: q, severity: "need" }),
      world,
    );
  ...
  if (/watch\s+(.+)/i.test(q) && !/watchboard|watching/.test(lower)) {
    ...
    if (name) {
      executeBuiltin(
        "board_upsert",
        JSON.stringify({ name, summary: `Watching ${name}.`, items: [] }),
        world,
      );
...
export function localFallback(userText: string, snap: Snapshot): string {
  ...
  if (/what do you remember|memories|memory/.test(q)) {
    if (!snap.memories.length)
      return "Nothing durable yet. Tell me to remember something, or add it in Memory.";
