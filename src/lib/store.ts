...
import { type ChatMessage, type ChatTool, completeTurn } from "./llm"
...
async function runToolsLoop(world: World, chat: ChatMessage[]): Promise<{ spoken: string; error?: string }> {
	...
	send: async (text) => {
		...
		if (triggered.length) {
			...
			world.snapshot.automations = world.snapshot.automations.map((a) =>
				triggered.some((t) => t.id === a.id) ? { ...a, lastRunAt: now, lastResult: "Triggered by phrase." } : a,
			)
		}
		...
		if (world.opened) get().openArtifact(world.opened)
		get().persist()
		const added = world.snapshot.inbox.filter((i) => !i.resolvedAt && !store.inbox.some((x) => x.id === i.id))
...
async function runTool(name: string, args: string, world: World): Promise<{ content: string; artifact?: Artifact }> {
