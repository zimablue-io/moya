...
export const BUILTIN_TOOLS = [
  {
    name: "memory_write",
    description:
      "Store or reinforce a durable memory about the user, a decision, a preference, or a project.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "board_upsert",
    description:
      "Create or update a watch board for any project. Use this instead of inventing live system state.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  },
  {
    name: "show_visual",
    description:
      "Show a visual: status grid, chart, diagram, or written brief. Use when words are not enough.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Summarize local transcripts into themes, decisions, and open loops. Call when asked to analyze.",
    inputSchema: {
      type: "object",
  ...
  if (name === "analyze_history") {
    ...
    const hours = s.timeLogs.reduce(
      (acc, t) => {
        acc[t.category] =
          (acc[t.category] ?? 0) +
          (new Date(t.endedAt).getTime() - new Date(t.startedAt).getTime()) / 3_600_000;
        return acc;
      },
    ...
    const body = [
      ...
      themes.length
        ? `Recurring: ${themes.join(", ")}.`
        : "Not enough transcript yet to see themes.",
...
function parseTrigger(args: Record<string, unknown>): AutomationTrigger | null {
  ...
  if (t === "interval")
    return { type: "interval", everyMinutes: Math.max(5, Number(args.everyMinutes ?? 60)) };
  if (t === "daily")
    return { type: "daily", hour: Number(args.hour ?? 9), minute: Number(args.minute ?? 0) };
  if (t === "phrase")
    return { type: "phrase", pattern: String(args.pattern ?? "").trim() || "remember" };
  return null;
}
