...
export function installPreviewHostBridge(options: PreviewHostBridgeOptions = {}): () => void {
	...
	try {
		...
		const alreadyTagged = current !== null && typeof current === "object" && Object.hasOwn(current, ROOT_STATE_KEY)
		if (!alreadyTagged) {
...
export function collectRoutePathsFromTree(routeTree: unknown): string[] {
	...
	const walk = (node: unknown) => {
		...
		const full =
			typeof record.fullPath === "string" ? record.fullPath : typeof record.path === "string" ? record.path : null
		if (full !== null && full !== "") {
