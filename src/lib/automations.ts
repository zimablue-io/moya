...
export const AUTOMATION_PRESETS: AutomationDraft[] = [
  ...
  },
  {
    name: "Watch loop",
    brief:
      "Scan boards. If something is blocked or needs the human, add an inbox item. Stay quiet if nothing changed.",
  ...
  },
  {
    ...
    brief:
      "Write a durable memory from what was just said. Prefer kind=decision or preference when it fits.",
...
export function makeAutomation(draft: AutomationDraft): Automation {
  return {
    ...
    lastRunAt:
      draft.trigger.type === "daily" || draft.trigger.type === "interval" ? nowIso() : null,
    lastResult: "",
    createdAt: nowIso(),
  };
}
...
export function isDue(auto: Automation, now = Date.now()): boolean {
  ...
  const pastSlot =
    d.getHours() > auto.trigger.hour ||
    (d.getHours() === auto.trigger.hour && d.getMinutes() >= auto.trigger.minute);
...
export function runLocalAutomation(auto: Automation, world: World): string {
  ...
  if (/inbox|loop|need|blocked|watch/.test(brief)) {
    const blocked = snap.boards.flatMap((b) =>
      b.items
        .filter((it) => it.needsInput || it.state === "blocked")
        .map((it) => `${b.name}: ${it.label}`),
    );
    if (blocked.length) {
      executeBuiltin(
        "inbox_add",
