...
export async function openMediaSettings(pane: "microphone" | "speech" = "microphone"): Promise<void> {
	if (!isDesktop()) return
	const { invoke } = await import("@tauri-apps/api/core")
	await invoke("open_media_settings", { pane })
}

export async function ensureMediaAccess(): Promise<{ ok: true } | { ok: false; message: string; fix: MicFix }> {
...
export async function ensureMicrophoneAccess(): Promise<{ ok: true } | { ok: false; message: string; fix: MicFix }> {
	let status = await mediaPermissionStatus()
...
function needsPrompt(status: MediaAuth): boolean {
	return status.microphone === "prompt" || (status.microphone === "granted" && status.speech === "prompt")
}
