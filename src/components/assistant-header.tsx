import { LayoutGrid, PanelRight, X } from "lucide-react"
...
export function AssistantHeader({ clock, pending, firstRun }: { clock: string; pending: number; firstRun: boolean }) {
	...
	const setConversationOpen = useApp((s) => s.setConversationOpen)

	return (
		<header className="pointer-events-none absolute inset-x-0 top-0 z-50 flex items-start justify-between px-4 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6">
			<div>
			...
			<div className="pointer-events-auto flex items-center gap-1">
				<button
					...
					className={cn(
						"grid size-11 place-items-center rounded-full text-muted-foreground transition-[color,background-color,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] outline-none ring-inset hover:bg-surface-2 hover:text-fg focus-visible:ring-3 focus-visible:ring-ring/50 active:scale-[0.96]",
						conversationOpen && "bg-surface-2 text-fg",
					)}
				>
				...
				<button
					...
					className={cn(
						"relative z-40 grid size-11 place-items-center rounded-full text-muted-foreground transition-[color,background-color,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] outline-none ring-inset hover:bg-surface-2 hover:text-fg focus-visible:ring-3 focus-visible:ring-ring/50 active:scale-[0.96]",
