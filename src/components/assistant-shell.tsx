...
export function AssistantShell() {
  ...
  const shown = settings.showCaptions
    ? interim || caption || (presence === "speaking" ? lastSpeak : "")
    : "";
    ...
    <main className="relative isolate min-h-dvh overflow-hidden bg-bg text-fg">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-40 flex items-start justify-between px-4 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6">
        <div>
          <p className="font-display text-2xl tracking-tight text-fg">
            {settings.agentName || "Moya"}
          </p>
          <p className="text-[11px] tracking-[0.18em] text-subtle uppercase tabular-nums">
            {clock}
          </p>
        </div>
        <button
        ...
        >
          ...
          {pending && !menuOpen ? (
            <span className="absolute top-2 right-2 size-1.5 rounded-full bg-warn" />
          ) : null}
        </button>
      </header>
      ...
      <PresenceCanvas
        ...
        gazeX={
          menuOpen
            ? 0.55
            : dialog === "settings" || dialog === "history" || dialog === "watch"
              ? 0.45
              : 0
        }
            ...
            <button
              ...
              className={cn(
                ...
                menuOpen
                  ...
                  : "translate-x-6 opacity-0 blur-[4px]",
              )}
            >
