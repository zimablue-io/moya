...
export function PresenceCanvas({
	...
	useEffect(() => {
		...
		const tick = (now: number) => {
			...
			draw(ctx, width, height, reduced ? t * 0.06 : t, L, attn.x, attn.y, voice.level, voice.bands, reduced)
...
function draw(
	...
	const tempo =
		live.state === "listening" ? 0.55 : live.state === "thinking" ? 0.72 : live.state === "speaking" ? 0.8 : 0.46
	const inhale =
	...
	for (let i = 0; i < rings; i++) {
		...
		for (let s = 0; s <= steps; s++) {
			...
			const speech = live.state === "speaking" ? (bands[s % bands.length] - 0.16) * (0.012 + depth * 0.01) : 0
