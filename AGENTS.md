...
- Voice mode is OpenAI Realtime over WebSocket (`src/lib/realtime-session.ts`). Default is Custom (empty URL). Desktop also offers Local (`huggingface/speech-to-speech` on `:8765`). Speakers come from the configured endpoint (`GET /tts/voices` when the URL is the xAI preset, `GET /voices` otherwise); hardcoded lists are fallback when that request is empty or 404. Do not invent extra list paths. Provider labels are `PROVIDER_PRESETS[id].label` (Voice Custom uses the same string as Model Custom). Hear this voice is the Play/Square `size="icon"` button on the same row as the Voice picker (`flex gap-2` in `SpokenVoice`) — HTTP TTS for the configured voice, not a text Preview below the row and not System TTS. Web and phone hide Local; leftover stored Local on those hosts remaps to Custom. llama.cpp is Settings → Model (on-device GGUF or optional llama-server URL); it is not a voice server. Moya does not start speech-to-speech or llama-server. Web does not offer Local voice, Ollama, llama.cpp-as-localhost, or on-device GGUF (`liveSettings` / `hostCaps` / `*ChoicesForHost`). Phone/tablet apps hide those sidecars too and offer `ondevice`.
