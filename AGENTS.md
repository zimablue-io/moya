...
**Voice is the product.** A turn that is only text is unfinished. Talk, Type, and Hear this voice speak through the configured Realtime voice. Caption-only is not done. Do not claim a turn works until it is heard, or a real speak error is shown.
...
- Home (`/`) is the assistant. Sign-in is optional; `/login` is not a gate. First open is a three-step dialog (provider, voice, standing brief). Settings stays closed until those three are written.
- Memory, transcript, routines, inbox, and settings persist locally (`src/lib/persist.ts`).
- Chat completion is `completeTurn` in `src/lib/llm.ts`. Default provider is on-device GGUF. Hosts without an in-process engine remap that to Custom OpenAI-compatible (`127.0.0.1:1234`), never xAI. Cloud and optional desktop sidecars (Ollama, llama-server) use `fetch`. Provider `ondevice` uses Tauri `invoke("llm_complete")` (in-process llama.cpp + a user-picked GGUF). Same `{ ok, content, toolCalls }` either way. Linked today: Mac Metal, iOS Metal, Android Vulkan. Windows/Linux stay a stub until a Vulkan/CPU link is proven. Desktop with a linked engine can Open a GGUF from disk (`hostCaps().pickGgufFromDisk`); phone/tablet download into app data. llama-server / Ollama stay optional HTTP on desktop OS. GGUF weights unload on hide-to-tray, Quit, leaving `ondevice`, and ~5 minutes idle. Picking a file does not mmap Metal; the first turn loads. First Talk after unload reloads.
- Voice mode is OpenAI Realtime over WebSocket (`src/lib/realtime-session.ts`). Default is Custom (empty URL). Desktop also offers Local (`huggingface/speech-to-speech` on `:8765`). Settings → Voice is not Settings → Model: Local is Base URL + Kokoro speaker (no Realtime model field, no API key). Custom adds Realtime model (`gpt-realtime` / `grok-voice-latest`) and key. Speakers: Kokoro for Local (the sidecar OpenAPI has no `GET /voices`), documented OpenAI Realtime names for `api.openai.com`, `GET /v1/tts/voices` only when the URL is the xAI preset. Hear this voice is the Play/Square `size="icon"` button on the same row as the Voice picker — a short Realtime `conversation.item.create` + `response.create` turn, not HTTP `/audio/speech` (the sidecar 404s that path). Typed turns speak that reply through the same Realtime path (`speakReply`); Talk already plays sidecar audio. Do not use System TTS. Web and phone hide Local; leftover stored Local on those hosts remaps to Custom. llama.cpp is Settings → Model (on-device GGUF or optional llama-server URL); it is not a voice server. Moya does not start speech-to-speech or llama-server. Web does not offer Local voice, Ollama, llama.cpp-as-localhost, or on-device GGUF (`liveSettings` / `hostCaps` / `*ChoicesForHost`). Phone/tablet apps hide those sidecars too and offer `ondevice`.
...
- Voice is the product. `scripts/voice-system.test.mjs` must stay red if Conversation speaker mixes with a system `voiceURI`, empty Local omits `af_heart`, Web Speech finals become realtime Voice turns, leftover cancelled audio plays, Settings stops using `src/lib/voice-contract.ts`, Voice still offers System TTS, or typed send skips `speakReply` / `prepareSpokenReply`. A typed-turn ui-visual that only waits for a caption is not a voice test — it must require a `/realtime` websocket. `.agents/rules/00-voice-is-the-product.md` is the always-on overlay.
