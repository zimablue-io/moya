...
- Chat completion is `completeTurn` in `src/lib/llm.ts`. Cloud and optional desktop sidecars (Ollama, llama-server) use `fetch`. Provider `ondevice` uses Tauri `invoke("llm_complete")` (in-process llama.cpp + a user-picked GGUF). Same `{ ok, content, toolCalls }` either way. Linked today: Mac Metal, iOS Metal, Android Vulkan. Windows/Linux stay a stub until a Vulkan/CPU link is proven. Desktop with a linked engine can Open a GGUF from disk (`hostCaps().pickGgufFromDisk`); phone/tablet download into app data. llama-server / Ollama stay optional HTTP on desktop OS.
- Voice mode is OpenAI Realtime over WebSocket (`src/lib/realtime-session.ts`). Backends: Local (`huggingface/speech-to-speech` on `:8765`), Grok, OpenAI, System. Hosts with in-process llama.cpp (`onDeviceLlm`: Mac `.app`, iOS, Android) hide Local and remap leftover `:8765` to System so Talk does not require speech-to-speech. Desktop without an engine (Windows/Linux stub) still offers Local. First open of `Moya.app` defaults to System. llama.cpp is Settings → Model (on-device GGUF or optional llama-server URL); it is not a voice server. Moya does not start speech-to-speech or llama-server. Web does not offer Local voice, Ollama, llama.cpp-as-localhost, or on-device GGUF (`liveSettings` / `hostCaps` / `*ChoicesForHost`). Phone/tablet apps hide those sidecars too and offer `ondevice`. Voice on mobile is Grok or System for this work.
