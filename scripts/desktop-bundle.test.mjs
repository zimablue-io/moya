...
test("desktop tray and autostart stay desktop-only; mobile uses the same llm commands", () => {
	...
	assert.equal(existsSync(join(tauriDir, "src/llm/pick.rs")), true)
	...
	assert.match(cargo, /target_os = "macos"/)
	...
	assert.match(cargo, /rfd/)
	assert.match(lib, /llm::llm_pick/)
	const engineMod = readFileSync(join(tauriDir, "src/llm/engine/mod.rs"), "utf8")
	assert.match(engineMod, /Windows and Linux/)
	assert.match(engineMod, /stub/)
	const pkg = JSON.parse(readFileSync(join(root, "package.json"), "utf8"))
